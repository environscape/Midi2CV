/*
  1.CVPreset1-4改为变量 可以被覆盖修改
  2.增加导入模式 通过长按按钮5s进入 灯闪烁
  3.增加串口通信，在导入模式如果收到数据则进行以下逻辑
  4.将串口数据存储于eeprom之中
  5.开机读取eeprom，如果没有则使用默认的配置
  6.在导入模式时，定时发送当前的eeprom配置，以供展示
  7.增加导入导出工具的日志发送接收
  8.导入工具增加下拉选择器等功能组件，可以保存临时配置等
*/
